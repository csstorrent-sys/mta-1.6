...
// If the pointer is null, the value is restored to the automatic one;
// otherwise it is set to the pointed value.
//
///////////////////////////////////////////////////////////////
                                                ...
                                                uint uiMaxCachedAllowed)
{
    ...
    // Do not evict models that GTA itself still marks as required or that are
    // still needed by native ped/vehicle streaming slots. This avoids cache
    // churn that shows up as micro-stutter when driving through busy areas.
...
bool CModelCacheManagerImpl::TryReleaseCachedModel(ushort usModelId, SModelCacheInfo& info, const char* contextTag, uint& uiNumModelsCachedHereOnly)
{
    ...
    if (bTrackedByNativePedSlots && GetNativeNumPedsLoaded() <= PED_STREAMING_FLOOR)
        return false;
